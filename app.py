...
def plot_chart(df, trades_df):
    ...
    if 'Div_Line_Price' in df.columns:
        fig.add_trace(go.Scatter(
            x=df.index,
            y=df['Div_Line_Price'],
            mode='lines',
...
def main():
    ...
    st.title("Sniper Terminal – RSI Bullish Divergence")
    st.caption("Strict swing‑low divergence • EMA200 + AVWAP trend filter • ATR‑based SL/TP • Full backtester")
