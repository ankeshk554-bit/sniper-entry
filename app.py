...
# Preserve the datetime index as a column before resetting
df = df.copy()
...
if run_btn:
    ...
    # Flatten MultiIndex columns produced by yfinance (must happen before indicators)
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(0)

    # Drop any duplicate columns that can appear after flattening
